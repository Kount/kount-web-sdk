...
export default function kountSDK(config, sessionID) {
  const sdk = {
    ...
    start(config, sessionID) {
      ...
      this.isDebugEnabled =
        typeof config.isDebugEnabled !== "undefined" &&
        typeof config.isDebugEnabled === "boolean" &&
      ...
      if (
        typeof configuredClientID === "undefined" ||
      ...
      if (
        typeof configuredIsSPA === "undefined" ||
        (configuredIsSPA !== true && configuredIsSPA !== false)
      ) {
      ...
      try {
        ...
        if (this.serverConfig.da.enabled) {
          if (config.triggers) {
            ...
            this._runDA(
              config.triggers,
              this.serverConfig.da.subdomain,
              this.serverConfig.da.orgId
            );
    ...
    async _getServerConfig() {
      ...
      try {
        ...
        const response = await this._wrapPromiseInTimeout(
          this.updateSDKServerConfigTimeoutInMS,
          fetch(url)
        );
        if (!response.ok) {
    ...
    _translateCollectorConfig(jsonConfig) {
      ...
      try {
        ...
        if (
          typeof jsonConfig.collection == "undefined" ||
          typeof jsonConfig.collection.feature_flags == "undefined"
        ) {
      ...
      if (typeof configuredHostname !== "undefined") {
        if (this._isHostnameValid(configuredHostname)) {
          ...
          if (configuredEnvironment) {
            this.log(
              `warning:both 'environment':${configuredEnvironment} and deprecated 'hostname':${configuredHostname} configs were specified. using hostname.`
            );
          }
    ...
    _communicateLatestSessionData() {
      try {
        ...
        const sessionIdInSessionStorage = sessionStorage.getItem(
          this.SESSION_STORAGE_KEY_SESSION_ID
        );
    ...
    async postChangeSession(sessionID, previousSessionID) {
      try {
        this.log(
          `postChangeSession running: newSession: ${sessionID} prevSession: ${previousSessionID}`
        );
        ...
        request.onreadystatechange = () => {
          if (request.readyState === XMLHttpRequest.DONE) {
            if (request.status === 200 || request.status === 201) {
              ...
              this.addError(
                `${calledFromFunc} unknown response: ${request.status}`
              );
        ...
        for (let i = 0; i < cookieList.length; i++) {
          ...
          if (elements.length === 2) {
            if (elements[0] === this.FPCV_COOKIE_NAME) {
              ...
              this.log(
                `getFPCVFromCookie: found new first party cookie: ${cookieValue}`
              );
              break;
            }
          }
          ...
          for (let i = 0; i < cookieList.length; i++) {
            ...
            if (regex.test(currentCookie) === true) {
              ...
              if (elements.length === 2) {
                [, cookieValue] = elements;
                this.log(
                  `getFPCVFromCookie: found old first party cookie: ${cookieValue}`
                );
    ...
    coordinateFirstPartyCookieValues() {
      ...
      if (fpcvFromCookie === "" && fpcvFromLocalStorage === "") {
        ...
        fpcvFromLocalStorage === fpcvFromCookie &&
        fpcvFromLocalStorage !== "" &&
        fpcvFromCookie !== ""
      ) {
        deducedFPCV = fpcvFromLocalStorage;
      } else if (
    ...
    runCollector() {
      ...
      try {
        ...
        const waitForFirstParty = (timeoutInMS, intervalBetweenChecksInMS) =>
          new Promise((resolve, reject) => {
            const checkForFirstParty = () => {
              if (
                sessionStorage.getItem(this.FPCV_SESSION_STORAGE_KEY) !== ""
              ) {
                this._createIframe();
                resolve();
        ...
        (async () => {
          waitForFirstParty(2000, 10)
            ...
            .catch(() =>
              this.log(`${functionName}: Invalid/Missing First Party cookie`)
            );
        })();
      } catch (e) {
        this.addError(`${functionName} error:${e}`);
    ...
    _runDA(triggers, subdomain, orgId) {
      ...
      try {
        ...
        daScript.setAttribute(
          "src",
          daUrl + "/in/web-sdk/v1/static/web-sdk.js"
        );
    ...
    AttachToForm(formID, options = new Map()) {
      ...
      if (
        options != null &&
        options.has("CustomFieldName") &&
        options.get("CustomFieldName").length > 0
      ) {
        this.log(
          `AttachToForm: Overriding decisionPointField name to: ${options.get(
            "CustomFieldName"
          )}`
        );
      ...
      if (form != null) {
        if (typeof form[decisionPointField] === "undefined") {
          ...
          this.log(
            `AttachToForm: Field ${decisionPointField} found, setting value to: ${this.sessionID}`
          );
          form[decisionPointField].setAttribute("value", this.sessionID);
        }

        this
          .log(`AttachToForm: Attached to form successfully using ${this.sessionID} \
                value in ${decisionPointField} hidden field.`);
      } else {
        this.addError(
          `AttachToForm: FormID: ${formID} is not valid. Skipping attachment to form and collection.`
        );
      }
    },
    ...
    NewSession(sessionID) {
      if (typeof sessionID === "undefined") {
        this.addError(
          "NewSession: Invalid sessionID.  You must set the 'sessionID' for collection. Disabling Kount SDK"
        );
      } else {
